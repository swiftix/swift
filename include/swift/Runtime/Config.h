...
// a runtime function. It should be used like:
...
// Map a calling convention to LLVM calling convention
...
// Both the wrapper and the implementation are hidden and
// can be directly referenced only inside the runtime library.
// User code accesses these runtime entries indirectly,
...
// Entry points are exported, because it should be possible
// to invoke them directly from the user code. But internal
// implementations do not need to be exported.
