...
public:
  enum FileType { AsmFile, ObjectFile, CFile,
                  SILFile, SwiftModuleFile, SIBFile, IRFile,
  };
...
public:
  ...
  static SWIFT *createSwift(const char *Argv0, std::string &Message,
                        const std::string              &SwiftBinary,
                        const std::vector<std::string> *Args = nullptr,
                        const std::vector<std::string> *SwiftArgs = nullptr,
                        bool UseIntegratedAssembler = false);

  static SILOpt *createSILOpt(const char *Argv0, std::string &Message,
                        const std::string              &SILOptBinary,
                        const std::vector<std::string> *Args = nullptr,
                        const std::vector<std::string> *SILOptArgs = nullptr,
                        bool UseIntegratedAssembler = false);
...
//
class SWIFT : public AbstractInterpreter {
  std::string SwiftPath;               // The path to the Swift executable.
  std::vector<std::string> ToolArgs; // Extra args to pass to Swift.
  bool UseIntegratedAssembler;
public:
  SWIFT(const std::string &swiftPath,
      const std::vector<std::string> *Args,
      bool useIntegratedAssembler)
    : SwiftPath(swiftPath),
      UseIntegratedAssembler(useIntegratedAssembler) {
  ...
  /// OutputCode - Compile the specified program from bitcode to code
  /// understood by the Swift driver.  If the code generator
  /// fails, it sets Error, otherwise, this function returns the type of code
  /// emitted.
  CC::FileType OutputCode(const std::string &Bitcode,
                           std::string &OutFile, std::string &Error,
...
//
class SILOpt : public AbstractInterpreter {
  std::string SILOptPath;               // The path to the Swift executable.
  std::vector<std::string> ToolArgs; // Extra args to pass to Swift.
  bool UseIntegratedAssembler;
public:
  SILOpt(const std::string &silOptPath,
      const std::vector<std::string> *Args,
      bool useIntegratedAssembler)
    : SILOptPath(silOptPath),
      UseIntegratedAssembler(useIntegratedAssembler) {
    ToolArgs.clear();
    if (Args) ToolArgs = *Args;
  }
  ...
  ArrayRef<std::string> getToolArgs() const {
    return ToolArgs;
  }

  StringRef getToolPath() const {
    return SILOptPath;
  }
