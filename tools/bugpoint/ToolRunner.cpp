...
//===----------------------------------------------------------------------===//
// Swift Implementation of AbstractIntepreter interface
//
CC::FileType SWIFT::OutputCode(const std::string &Bitcode,
                              std::string &OutputAsmFile, std::string &Error,
                              unsigned Timeout, unsigned MemoryLimit) {
  const char *Suffix = (UseIntegratedAssembler ? ".llc.o" : ".llc.s");
  ...
  OutputAsmFile = UniqueFile.str();
  std::vector<const char *> SwiftArgs;
  SwiftArgs.push_back(SwiftPath.c_str());

  // Add any extra LLC args.
  for (unsigned i = 0, e = ToolArgs.size(); i != e; ++i)
    SwiftArgs.push_back(ToolArgs[i].c_str());

  SwiftArgs.push_back("-o");
  SwiftArgs.push_back(OutputAsmFile.c_str()); // Output to the Asm file
  SwiftArgs.push_back(Bitcode.c_str());      // This is the input bitcode

  if (UseIntegratedAssembler)
    SwiftArgs.push_back("-filetype=obj");

  SwiftArgs.push_back (nullptr);

  outs() << (UseIntegratedAssembler ? "<llc-ia>" : "<llc>");
  outs().flush();
  DEBUG(errs() << "\nAbout to run:\t";
        for (unsigned i = 0, e = SwiftArgs.size()-1; i != e; ++i)
          errs() << " " << SwiftArgs[i];
        errs() << "\n";
        );
  if (RunProgramWithTimeout(SwiftPath, &SwiftArgs[0],
                            "", "", "",
                            Timeout, MemoryLimit))
    Error = ProcessFailure(SwiftPath, &SwiftArgs[0],
                           Timeout, MemoryLimit);
  return UseIntegratedAssembler ? CC::ObjectFile : CC::AsmFile;
}

void SWIFT::compileProgram(const std::string &Bitcode, std::string *Error,
                         unsigned Timeout, unsigned MemoryLimit) {
  std::string OutputAsmFile;
  OutputCode(Bitcode, OutputAsmFile, *Error, Timeout, MemoryLimit);
  sys::fs::remove(OutputAsmFile);
...
int SWIFT::ExecuteProgram(const std::string &Bitcode,
                        const std::vector<std::string> &Args,
                        const std::string &InputFile,
                        const std::string &OutputFile,
                        std::string *Error,
                        const std::vector<std::string> &ArgsForSwift,
                        const std::vector<std::string> &SharedLibs,
                        unsigned Timeout,
                        unsigned MemoryLimit) {
  ...
  std::vector<std::string> SwiftArgs(ArgsForSwift);
  SwiftArgs.insert(SwiftArgs.end(), SharedLibs.begin(), SharedLibs.end());

#if 0
  // Assuming Swift worked, compile the result with CC and run it.
  return cc->ExecuteProgram(OutputAsmFile, Args, FileKind,
                             InputFile, OutputFile, Error, SwiftArgs,
                             Timeout, MemoryLimit);
#endif
...
/// createSwift - Try to find the Swift executable
///
SWIFT *AbstractInterpreter::createSwift(const char *Argv0,
                                    std::string &Message,
                                    const std::string &SwiftBinary,
                                    const std::vector<std::string> *Args,
                                    const std::vector<std::string> *SwiftArgs,
                                    bool UseIntegratedAssembler) {
  std::string SwiftPath =
      PrependMainExecutablePath("swift", Argv0, (void *)(intptr_t) & createSwift);
  if (SwiftPath.empty()) {
    Message = "Cannot find `swift' in executable directory!\n";
    return nullptr;
  }

  Message = "Found swift: " + SwiftPath + "\n";
  return new SWIFT(SwiftPath, Args, UseIntegratedAssembler);
}

//===----------------------------------------------------------------------===//
// sil-opt Implementation of AbstractIntepreter interface
//
CC::FileType SILOpt::OutputCode(const std::string &Bitcode,
                              std::string &OutputAsmFile, std::string &Error,
                              unsigned Timeout, unsigned MemoryLimit) {
  const char *Suffix = (UseIntegratedAssembler ? ".llc.o" : ".llc.s");
  ...
  OutputAsmFile = UniqueFile.str();
  std::vector<const char *> SILOptArgs;
  SILOptArgs.push_back(SILOptPath.c_str());

  // Add any extra LLC args.
  for (unsigned i = 0, e = ToolArgs.size(); i != e; ++i)
    SILOptArgs.push_back(ToolArgs[i].c_str());

  SILOptArgs.push_back("-o");
  SILOptArgs.push_back(OutputAsmFile.c_str()); // Output to the Asm file
  SILOptArgs.push_back(Bitcode.c_str());      // This is the input bitcode

  if (UseIntegratedAssembler)
    SILOptArgs.push_back("-filetype=obj");

  SILOptArgs.push_back (nullptr);

  outs() << (UseIntegratedAssembler ? "<llc-ia>" : "<llc>");
  outs().flush();
  DEBUG(errs() << "\nAbout to run:\t";
        for (unsigned i = 0, e = SILOptArgs.size()-1; i != e; ++i)
          errs() << " " << SILOptArgs[i];
        errs() << "\n";
        );
  if (RunProgramWithTimeout(SILOptPath, &SILOptArgs[0],
                            "", "", "",
                            Timeout, MemoryLimit))
    Error = ProcessFailure(SILOptPath, &SILOptArgs[0],
                           Timeout, MemoryLimit);
  return UseIntegratedAssembler ? CC::ObjectFile : CC::AsmFile;
}

void SILOpt::compileProgram(const std::string &Bitcode, std::string *Error,
                         unsigned Timeout, unsigned MemoryLimit) {
  std::string OutputAsmFile;
  OutputCode(Bitcode, OutputAsmFile, *Error, Timeout, MemoryLimit);
  sys::fs::remove(OutputAsmFile);
...
int SILOpt::ExecuteProgram(const std::string &Bitcode,
                        const std::vector<std::string> &Args,
                        const std::string &InputFile,
                        const std::string &OutputFile,
                        std::string *Error,
                        const std::vector<std::string> &ArgsForSILOpt,
                        const std::vector<std::string> &SharedLibs,
                        unsigned Timeout,
                        unsigned MemoryLimit) {
  ...
  std::vector<std::string> SILOptArgs(ArgsForSILOpt);
  SILOptArgs.insert(SILOptArgs.end(), SharedLibs.begin(), SharedLibs.end());
...
/// createSILOpt - Try to find the sil-opt executable
///
SILOpt *AbstractInterpreter::createSILOpt(const char *Argv0,
                                    std::string &Message,
                                    const std::string &SILOptBinary,
                                    const std::vector<std::string> *Args,
                                    const std::vector<std::string> *SILOptArgs,
                                    bool UseIntegratedAssembler) {
  std::string SILOptPath =
      PrependMainExecutablePath("sil-opt", Argv0, (void *)(intptr_t) & createSILOpt);
  if (SILOptPath.empty()) {
    Message = "Cannot find `sil-opt' in executable directory!\n";
    return nullptr;
  }

  Message = "Found sil-opt: " + SILOptPath + "\n";
  return new SILOpt(SILOptPath, Args, UseIntegratedAssembler);
}
