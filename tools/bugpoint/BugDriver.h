...
class SwiftBugDriver {
  LLVMContext& Context;
  const char *ToolName;            // argv[0] of bugpoint
  std::string ReferenceOutputFile; // Name of `good' output file
  swift::SILModule *Program;             // The raw program, linked together
  std::vector<std::string> PassesToRun;
  AbstractInterpreter *Interpreter;   // How to run the program
  AbstractInterpreter *SafeInterpreter;  // To generate reference output, etc.
  CC *cc;
...
public:
  SwiftBugDriver(const char *toolname, bool find_bugs,
            unsigned timeout, unsigned memlimit, bool use_valgrind,
            LLVMContext& ctxt);
  ~SwiftBugDriver();
  ...
  /// runPasses - Run all of the passes in the "PassesToRun" list, discard the
  /// output, and return true if any of the passes crashed.
  bool runPasses(swift::SILModule *M) const {
    return runPasses(M, PassesToRun);
  }

  swift::SILModule *getProgram() const { return Program; }

  /// swapProgramIn - Set the current module to the specified module, returning
  /// the old one.
  swift::SILModule *swapProgramIn(swift::SILModule *M) {
    swift::SILModule *OldProgram = Program;
    Program = M;
    return OldProgram;
  }
  ...
  /// method to update bugdriver with it.  This deletes the old module and sets
  /// the specified one as the current program.
  void setNewProgram(swift::SILModule *M);

  /// compileProgram - Try to compile the specified module, returning false and
  /// setting Error if an error occurs.  This is used for code generation
  /// crash testing.
  ///
  void compileProgram(swift::SILModule *M, std::string *Error) const;

  /// executeProgram - This method runs "Program", capturing the output of the
  /// program to a file.  A recommended filename may be optionally specified.
  ///
  std::string executeProgram(const swift::SILModule *Program,
                             std::string OutputFilename,
                             std::string Bitcode,
                             const std::string &SharedObjects,
                             AbstractInterpreter *AI,
  ...
  /// Error.
  ///
  std::string executeProgramSafely(const swift::SILModule *Program,
                                   std::string OutputFile,
                                   std::string *Error) const;
  ...
  /// this function.
  ///
  bool createReferenceFile(swift::SILModule *M, const std::string &Filename
                                            = "bugpoint.reference.out-%%%%%%%");
  ...
  /// generator (e.g., llc crashes), this will return -1 and set Error.
  ///
  bool diffProgram(const swift::SILModule *Program,
                   const std::string &BitcodeFile = "",
                   const std::string &SharedObj = "",
                   bool RemoveBitcode = false,
                   std::string *Error = nullptr) const;
  ...
  /// "bugpoint-ID.bc".
  ///
  void EmitProgressBitcode(const swift::SILModule *M, const std::string &ID,
                           bool NoFlyer = false) const;
  ...
  /// value. The modified module is then returned.
  ///
  std::unique_ptr<swift::SILModule> deleteInstructionFromProgram(const swift::SILInstruction *I,
                                                       unsigned Simp);
  ...
  /// modify how the code behaves.
  ///
  std::unique_ptr<swift::SILModule> performFinalCleanups(swift::SILModule *M,
                                               bool MayModifySemantics = false);

  /// Given a module, extract up to one loop from it into a new function. This
  /// returns null if there are no extractable loops in the program or if the
  /// loop extractor crashes.
  std::unique_ptr<swift::SILModule> extractLoop(swift::SILModule *M);
  ...
  /// some reason (ie the implementation is buggy), this function should return
  /// null, otherwise it returns a new Module.
  std::unique_ptr<swift::SILModule>
  extractMappedBlocksFromModule(const std::vector<swift::SILBasicBlock *> &BBs,
                                swift::SILModule *M);
  ...
  /// attempt to track down a crashing pass if one exists, and this method will
  /// never return null.
  std::unique_ptr<swift::SILModule> runPassesOn(swift::SILModule *M,
                                      const std::vector<std::string> &Passes,
                                      bool AutoDebugCrashes = false,
                                      unsigned NumExtraArgs = 0,
                                      const char *const *ExtraArgs = nullptr);
  ...
  /// to pass to the child bugpoint instance.
  ///
  bool runPasses(swift::SILModule *Program,
                 const std::vector<std::string> &PassesToRun,
                 std::string &OutputFilename, bool DeleteOutput = false,
                 bool Quiet = false, unsigned NumExtraArgs = 0,
                 const char * const *ExtraArgs = nullptr) const;
  ...
  /// bitcode file.  If an error occurs, true is returned.
  ///
  bool writeProgramToFile(const std::string &Filename, const swift::SILModule *M) const;
  bool writeProgramToFile(const std::string &Filename, int FD,
                          const swift::SILModule *M) const;
...
private:
  ...
  /// input (true = crashed).
  ///
  bool runPasses(swift::SILModule *M,
                 const std::vector<std::string> &PassesToRun,
                 bool DeleteOutput = true) const {
    std::string Filename;
    return runPasses(M, PassesToRun, Filename, DeleteOutput);
...
///  Given a Swift or SIL input filename, parse and return it, or return
///  null if not possible.
///
std::unique_ptr<swift::SILModule> parseSILInputFile(StringRef InputFilename,
                                             LLVMContext &ctxt,
...
void PrintFunctionList(const std::vector<swift::SILFunction*> &Funcs);
...
void DeleteFunctionBody(swift::SILFunction *F);
...
std::unique_ptr<swift::SILModule>
SplitFunctionsOutOfModule(swift::SILModule *M, const std::vector<swift::SILFunction *> &F,
                          ValueToValueMapTy &VMap);
