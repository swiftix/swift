...
namespace {
  ...
  //
  enum OutputType {
    AutoPick, RunLLI, RunJIT, RunLLC, RunLLCIA, RunSILOpt, RunSwift,
    RunSwiftc, LLC_Safe, CompileCustom, Custom
  };
  ...
  InterpreterSel(cl::desc("Specify the \"test\" i.e. suspect back-end:"),
                 cl::values(clEnumValN(AutoPick, "auto", "Use best guess"),
                            ...
                            clEnumValN(RunSILOpt, "run-sil-opt", "Compile with sil-opt"),
                            clEnumValN(RunSwift, "run-swift", "Compile with swift"),
                            clEnumValN(RunSwiftc, "run-swiftc", "Compile with swiftc"),
  ...
  SafeInterpreterSel(cl::desc("Specify \"safe\" i.e. known-good backend:"),
              cl::values(clEnumValN(AutoPick, "safe-auto", "Use best guess"),
                         ...
                         clEnumValN(RunSILOpt, "safe-run-sil-opt", "Compile with sil-opt"),
                         clEnumValN(RunSwiftc, "safe-run-swiftc", "Compile with swiftc"),
  ...
  cl::opt<std::string>
  CustomCompileCommand("compile-command", cl::init("sil-opt"),
...
namespace {
  ...
  cl::opt<std::string>
  SwiftBinary("swift", cl::init(""), cl::desc("The swift binary to use."));

  cl::list<std::string>
  SwiftToolArgv("swift-tool-args", cl::Positional,
              cl::desc("<swift-tool arguments>..."),
              cl::ZeroOrMore, cl::PositionalEatsArgs);

  cl::opt<std::string>
  SILOptBinary("sil-opt", cl::init(""), cl::desc("The sil-opt binary to use."));

  cl::list<std::string>
  SILOptToolArgv("sil-opt-tool-args", cl::Positional,
              cl::desc("<sil-opt-tool arguments>..."),
              cl::ZeroOrMore, cl::PositionalEatsArgs);
...
bool BugDriver::initializeExecutionEnvironment() {
  ...
  if (SwiftBinary.empty()) {
    if (sys::findProgramByName("swift"))
      SwiftBinary = "swift";
  }
  ...
  case RunSILOpt:
    Interpreter = AbstractInterpreter::createSILOpt(getToolName(), Message,
                                                 SILOptBinary, &ToolArgv,
                                                 &SILOptToolArgv,
                                                 InterpreterSel == RunSwift);
    break;

  case RunSwift:
  case RunSwiftc:
    Interpreter = AbstractInterpreter::createSwift(getToolName(), Message,
                                                 SwiftBinary, &ToolArgv,
                                                 &SwiftToolArgv,
                                                 InterpreterSel == RunSwift);
    break;
  case CompileCustom:
...
bool SwiftBugDriver::initializeExecutionEnvironment() {
  outs() << "Initializing execution environment: ";
...
void SwiftBugDriver::compileProgram(swift::SILModule *M, std::string *Error) const {
#if 0
  // Emit the program to a bitcode file...
  SmallString<128> BitcodeFile;
  int BitcodeFD;
...
std::string SwiftBugDriver::executeProgram(const swift::SILModule *Program,
                                      std::string OutputFile,
                                      std::string BitcodeFile,
                                      const std::string &SharedObj,
                                      AbstractInterpreter *AI,
...
std::string SwiftBugDriver::executeProgramSafely(const swift::SILModule *Program,
                                            std::string OutputFile,
                                            std::string *Error) const {
  return executeProgram(Program, OutputFile, "", "", SafeInterpreter, Error);
}
...
std::string SwiftBugDriver::compileSharedObject(const std::string &BitcodeFile,
                                           std::string &Error) {
  assert(Interpreter && "Interpreter should have been created already!");
#if 0
  std::string OutputFile;
...
bool SwiftBugDriver::createReferenceFile(swift::SILModule *M, const std::string &Filename) {
  std::string Error;
  compileProgram(Program, &Error);
  if (!Error.empty())
    return false;
...
bool SwiftBugDriver::diffProgram(const swift::SILModule *Program,
                            const std::string &BitcodeFile,
                            const std::string &SharedObject,
                            bool RemoveBitcode,
                            std::string *ErrMsg) const {
...
bool SwiftBugDriver::isExecutingJIT() {
  return InterpreterSel == RunJIT;
}
