...
static cl::opt<bool>
OptLevelO("O",
           cl::desc("Optimization level O. Identical to 'swiftc -O'"));

static cl::opt<bool>
OptLevelOunchecked("Ounchecked",
           cl::desc("Optimization level Ounchecked. Identical to 'swiftc -Ounchecked'"));

static cl::opt<bool>
OptLevelOnone("Onone",
           cl::desc("Optimization level Onone. Identical to 'swiftc -Onone'"));
...
namespace {
  ...
  class AddToSwiftDriver : public legacy::FunctionPassManager {
    SwiftBugDriver &D;
  public:
    AddToSwiftDriver(SwiftBugDriver &_D) : FunctionPassManager(nullptr), D(_D) {}
...
int main(int argc, char **argv) {
  ...
  cl::ParseCommandLineOptions(argc, argv,
                              "SIL/LLVM automatic testcase reducer. See\nhttp://"
  ...
  if (StringRef(argv[0]).find("swift-bugpoint", 0) == StringRef::npos) {
  // Initialize passes
  PassRegistry &Registry = *PassRegistry::getPassRegistry();
  initializeCore(Registry);
  initializeScalarOpts(Registry);
...
// avoid filling up the disk, we prevent it
    ...
    SwiftBugDriver D(argv[0], FindBugs, TimeoutValue, MemoryLimit, UseValgrind,
                     Context);

    if (D.addSources(InputFilenames))
      return 1;

    AddToSwiftDriver PM(D);
