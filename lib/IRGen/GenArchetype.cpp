...
// FIXME: We need to express that it is a POD of an unknown size.
// I guess, it is an address-only type.
// We need something like WitnessSizedTypeInfo which is POD and bitwise-copyable.
class NonFixedSizeArchetypeTypeInfo
  : public PODSingleScalarTypeInfo<NonFixedSizeArchetypeTypeInfo, LoadableTypeInfo>,
    public ArchetypeTypeInfoBase
{
  NonFixedSizeArchetypeTypeInfo(llvm::Type *type, Size size, Alignment align,
                             const SpareBitVector &spareBits,
                             ArrayRef<ProtocolEntry> protocols)
      : PODSingleScalarTypeInfo(type, size, spareBits, align),
        ArchetypeTypeInfoBase(this + 1, protocols) {}
...
public:
  static const NonFixedSizeArchetypeTypeInfo *
  create(llvm::Type *type, Size size, Alignment align,
         const SpareBitVector &spareBits, ArrayRef<ProtocolEntry> protocols) {
    void *buffer = operator new(sizeof(NonFixedSizeArchetypeTypeInfo) +
                                protocols.size() * sizeof(ProtocolEntry));
    return ::new (buffer)
        NonFixedSizeArchetypeTypeInfo(type, size, align, spareBits, protocols);
  }
};
