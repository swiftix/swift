...
void UsedTypesCollector::collect(CanType Ty) {
  ...
  while(auto MT = dyn_cast<AnyMetatypeType>(Ty)) {
    Ty = MT->getInstanceType().getCanonicalTypeOrNull();
  ...
  if (NTD) {
    ...
    for (auto P : Protocols) {
      DEBUG(llvm::dbgs() << "Adding protocol of " << NTD->getName() << ": "
                         << P->getNameStr() << "\n");
      collect(P->getDeclaredInterfaceType().getCanonicalTypeOrNull());
    }
#if 0
    ...
    if (auto CD = Ty->getClassOrBoundGenericClass()) {
      ...
      if (Superclass) {
        DEBUG(llvm::dbgs() << "Adding superclass of " << CD->getName() << ": "
                           << Superclass->getAnyNominal()->getNameStr()
                           << "\n");
        collect(Superclass.getCanonicalTypeOrNull());
      }
    }
    ...
    if (auto BGT = dyn_cast<BoundGenericType>(Ty)) {
      for (auto GA : BGT->getGenericArgs()) {
        collect(GA.getCanonicalTypeOrNull());
      }
    }
  ...
  if (FnTy) {
    ...
    Ty.visit([this, &Ty](Type ty) {
      if (ty.getCanonicalTypeOrNull() == Ty)
        return;
      if (UsedTypes.count(ty.getCanonicalTypeOrNull()))
        return;
      collect(ty.getCanonicalTypeOrNull());
    });
  ...
  for (auto Req : Requirements) {
    ...
    default:
      ...
      //if (FirstNTD) {
        collect(FirstTy.getCanonicalTypeOrNull());
#if 0
        auto Result = UsedTypes.insert(
            FirstNTD->getDeclaredType().getCanonicalTypeOrNull());
        if (Result.second) {
      ...
      //}
      //if (SecondNTD) {
        collect(SecondTy.getCanonicalTypeOrNull());
#if 0
        auto Result = UsedTypes.insert(
            SecondNTD->getDeclaredType().getCanonicalTypeOrNull());
        if (Result.second) {
          llvm::dbgs() << "Found a new type in function type: "
...
void UsedTypesCollector::collectMetatypes(CanType Ty) {
  ...
  if (NTD) {
    ...
    if (auto BGT = dyn_cast<BoundGenericType>(Ty)) {
      for (auto GA : BGT->getGenericArgs()) {
        collectMetatypes(GA.getCanonicalTypeOrNull());
      }
    }
  }
}
...
void UsedTypesCollector::collect(const SILWitnessTable &WT) {
  for (auto Witness : WT.getEntries()) {
    ...
    case swift::SILWitnessTable::BaseProtocol: {
      ...
      collect(
          baseProtoWitness.Witness->getType().getCanonicalTypeOrNull());
      break;
    }
...
void UsedTypesCollector::collectMetatypes(const SILInstruction &I) {
  ...
  if (ApplySite AI = ApplySite::isa(const_cast<SILInstruction *>(&I))) {
    ...
    for (auto Sub : Subs) {
      collectMetatypes(Sub.getReplacement().getCanonicalTypeOrNull());
    }
    return;
  }
      ...
      if (auto NTD = DNT->getNominalOrBoundGenericNominal()) {
        ...
        for (auto C : Conformances) {
          ...
          if (!WT) {
            ...
            if (WT) {
              DEBUG(llvm::dbgs() << "Loaded SIL witness table " << WT->getName()
                                 << "\n";
                    WT->dump());
              UsedTypes.collect(*WT);
            }
          } else {
