    ...
    for (auto DNT : Types) {
      ...
      if (auto NTD = DNT->getNominalOrBoundGenericNominal()) {
        ...
        for (auto C : Conformances) {
          ...
          if (!WT) {
            ...
            DEBUG(llvm::dbgs()
                  << "Created a declration for witness table "
                  << WT->getName() << "\n"
                  << "for conformance ";
                  C->dump());
      ...
      if (auto CD = DNT->getClassOrBoundGenericClass()) {
        ...
        for (auto Member : CD->getMembers()) {
          ...
          bool IsObjC = AFD->isObjC();
          auto *FD = dyn_cast<FuncDecl>(AFD);

          if (!IsObjC && FD && FD->isAccessor() &&
              FD->getAccessorStorageDecl()->isObjC()) {
            IsObjC = true;
          }
          // The functions we are interested in are @objc functions
          // and destructors.
          if (Kind != SILDeclRef::Kind::Deallocator && !IsObjC)
            continue;
          // Only load constructors and destructors.
          // if (Kind == SILDeclRef::Kind::Func)
          //  continue;
          if (Kind != SILDeclRef::Kind::Func) {
            SILDeclRef MemberRef(AFD, Kind, ResilienceExpansion::Minimal,
