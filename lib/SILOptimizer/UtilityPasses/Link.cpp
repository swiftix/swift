...
static void collectUsedTypes(CanType Ty, llvm::DenseSet<CanType> &UsedTypes) {
  ...
  if (NTD) {
    ...
    for (auto P : Protocols) {
     llvm::dbgs() << "Adding protocol of " << NTD->getName() << ": "
                     << P->getNameStr() << "\n";
     collectUsedTypes(P->getDeclaredInterfaceType().getCanonicalTypeOrNull(),
                       UsedTypes);
    }
#if 0
    ...
    if (auto CD = Ty->getClassOrBoundGenericClass()) {
      ...
      if (Superclass) {
        ...
        collectUsedTypes(Superclass.getCanonicalTypeOrNull(), UsedTypes);
      }
    }
    ...
    if (auto BGT = dyn_cast<BoundGenericType>(Ty)) {
      for (auto GA : BGT->getGenericArgs()) {
        collectUsedTypes(GA.getCanonicalTypeOrNull(), UsedTypes);
      }
    }
  ...
  if (FnTy) {
    ...
    Ty.visit([&UsedTypes, &Ty](Type ty) {
      ...
      if (UsedTypes.count(ty.getCanonicalTypeOrNull()))
        return;
      collectUsedTypes(ty.getCanonicalTypeOrNull(), UsedTypes);
    });
  ...
  for (auto Req : Requirements) {
    ...
    default:
      ...
      //if (FirstNTD) {
        collectUsedTypes(FirstTy.getCanonicalTypeOrNull(), UsedTypes);
#if 0
      ...
      //}
      //if (SecondNTD) {
        collectUsedTypes(SecondTy.getCanonicalTypeOrNull(), UsedTypes);
#if 0
        auto Result = UsedTypes.insert(
              ...
              for (auto Witness : WT->getEntries()) {
                ...
                case swift::SILWitnessTable::AssociatedTypeProtocol: {
                  ...
                  if (assocProtoWitness.Witness.isConcrete())
                    collectUsedTypes(assocProtoWitness.Witness.getConcrete()->getType().getCanonicalTypeOrNull(), UsedTypes);
                  break;
                }
                case swift::SILWitnessTable::BaseProtocol: {
                  auto &baseProtoWitness = Witness.getBaseProtocolWitness();
                  collectUsedTypes(baseProtoWitness.Witness->getType().getCanonicalTypeOrNull(), UsedTypes);
                  break;
                }
