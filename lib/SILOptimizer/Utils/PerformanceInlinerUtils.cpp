...
SILFunction *swift::getEligibleFunction(FullApplySite AI,
  ...
  SWIFT_DEFER {
    // If the callee cannot be ever inlined, mark it as such.
    // TODO: Introduce a different way to mark the callee, which is
    // different from a user-defined @inline(never) attribute?
    //if (!EligibleCallee && Callee->getInlineStrategy() != NoInline)
    //  Callee->setInlineStrategy(NoInline);
  };
