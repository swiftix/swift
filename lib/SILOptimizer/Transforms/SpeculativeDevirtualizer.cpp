...
createPolymorphicCacheThunk(FullApplySite &AI, SILType SubType,
  ...
  if (!AI.hasSubstitutions() &&
    ...
    while (Ty.is<AnyMetatypeType>()) {
      Ty = Ty.getMetatypeInstanceType(M);
      Buffer << "_metatype";
    }

    Buffer << "_trampoline_for_" << Ty.getNominalOrBoundGenericNominal()->getNameStr();
    auto PolymorphicCacheThunkName = Buffer.str();
    ...
    PolymorphicCacheThunk = M.getOrCreateFunction(
        AI.getLoc(), PolymorphicCacheThunkName, SILLinkage::Private, FnType,
