...
runOnFunctionRecursively(SILFunction *F, FullApplySite AI,
  ...
  for (auto FI = F->begin(), FE = F->end(); FI != FE; ++FI) {
    for (auto I = FI->begin(), E = FI->end(); I != E; ++I) {
      ...
      F->getModule().registerDeleteNotificationHandler(&OpenedArchetypes);
      // The callee only needs to know about opened archetypes used in
      // the substitution list.
      OpenedArchetypes.registerUsedOpenedArchetypes(InnerAI.getInstruction());

      SILInliner Inliner(*F, *CalleeFunction,
                         SILInliner::InlineKind::MandatoryInline, ContextSubs,
                         ApplySubs, OpenedArchetypes);
